...
def _print_banner(gateway: str) -> None:
    ...
    # ── PICO-8 像素葫芦 Logo（14列 × 8行）────────────────────────────────
    # PICO-8 调色板：橙 o / 深橙 d / 亮黄 l / 浅绿 g / 深绿 G / 棕 b / 深棕 B
    _O = "\033[38;5;214m"   # 橙（葫芦主体）
    _D = "\033[38;5;130m"   # 深橙（轮廓）
    _L = "\033[38;5;226m"   # 亮黄（高光点）
    _G = "\033[38;5;85m"    # 浅绿（叶子）
    _g = "\033[38;5;34m"    # 深绿（叶影）
    _b = "\033[38;5;131m"   # 棕（葫芦口）
    _B = "\033[38;5;94m"    # 深棕（口影）
    _W = "\033[38;5;255m"   # 白（叶脉）
    _X = "\033[0m"          # 重置

    def _p(ch): return f"{ch}█{_X}" if ch else " "

    GOURD_LINES = [
        "     " + _p(_G) + _p(_W) + _p(_G) + _p(_G) + "           ",
        "   " + _p(_g) + _p(_g) + _p(_G) + _p(_G) + _p(_G) + _p(_g) + _p(_g) + "        ",
        "  " + _p(_b) + _p(_B) + _p(_b) + _p(_b) + _p(_b) + _p(_b) + _p(_b) + _p(_B) + _p(_b) + "       ",
        " " + _p(_b) + _p(_b) + _p(_b) + _p(_b) + _p(_b) + _p(_b) + _p(_b) + _p(_b) + _p(_b) + _p(_b) + _p(_b) + _p(_b) + "      ",
        " " + _p(_O) + _p(_D) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_D) + "      ",
        _p(_O) + _p(_D) + _p(_O) + _p(_O) + _p(_O) + _p(_L) + _p(_O) + _p(_L) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_D) + _p(_O),
        _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_O),
        " " + _p(_O) + _p(_D) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_O) + _p(_D) + _p(_O) + "     ",
    ]
